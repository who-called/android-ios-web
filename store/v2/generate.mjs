#!/usr/bin/env node
/**
 * Générateur de visuels stores v2 pour Who Called — zéro dégradé.
 *
 * Différences avec la v1 (store/generate.mjs) :
 *   - Tous les linearGradient supprimés : fonds, header, jauge → aplats.
 *   - Header in-app repensé : surface blanche, titre encre, pastille de
 *     statut émeraude, icône dans un disque teinté (au lieu du bandeau
 *     dégradé bleu → nuit).
 *   - Deux alternatives de header exportées pour comparaison :
 *     android/header-alt-surface.png et android/header-alt-bleu-plat.png.
 *
 *   node store/v2/generate.mjs
 */
...
console.log(`OK — ${rendered} PNG rendus dans store/v2/png/ (SVG maîtres dans store/v2/svg/)`);
