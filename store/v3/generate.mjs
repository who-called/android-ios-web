#!/usr/bin/env node
/**
 * Générateur de visuels stores v3 pour Who Called — zéro dégradé + mini-jeux.
 *
 * Différences avec la v2 (store/v2/generate.mjs) :
 *   - Les deux mini-jeux (DEFENSE, arcade · TRACE, puzzle quotidien)
 *     apparaissent : slide dédié 07-jeux (+ variante éditoriale B4-jeux),
 *     onglet « Jeux » dans la barre de navigation des mockups, carte
 *     discrète sur l'écran d'accueil, mention dans les feature graphics
 *     et les listes de features (06-marque, B1-hero).
 *
 *   node store/v3/generate.mjs
 */
...
// ---------------------------------------------------------------------------
// Palette — uniquement des aplats
// ---------------------------------------------------------------------------
const C = {
  blue: "#2563EB",
  blueBright: "#3B82F6",
  night: "#17233E",     // remplace le dégradé bgNight
  nightDark: "#101F3C", // écran d'appel (aplat)
  nightBand: "#0C1526",
  amber: "#F59E0B",
  emerald: "#10B981",
  coral: "#EF4444",
  ...
  tint: "#EFF5FF",
  muted: "#64748B",
  skyFlat: "#E4EEFD",   // remplace le dégradé bgLight
  // Palette des mini-jeux (DefenseEngine / TraceEngine)
  gameCyan: "#2BE0C6",
  gameBlue: "#4C9BFF",
  gameSpam: "#FF5D6C",
  gameSafe: "#37E29A",
...
const ICONS = {
  ...
  gamepad: `<rect x="2.6" y="7.5" width="18.8" height="10" rx="5" fill="none" stroke-width="2.1"/><path d="M8 10.4v4.2M5.9 12.5h4.2" stroke-width="1.9" stroke-linecap="round"/><circle cx="15.2" cy="11" r="1.25" stroke="none"/><circle cx="17.8" cy="14" r="1.25" stroke="none"/>`,
  trophy: `<path d="M8 4h8v5.2a4 4 0 0 1-8 0z" fill="none" stroke-width="2.1" stroke-linejoin="round"/><path d="M8 5.2H4.8V7A3.2 3.2 0 0 0 8 10.2M16 5.2h3.2V7A3.2 3.2 0 0 1 16 10.2M12 13.2v2.6M8.8 19.4h6.4M10.4 15.8h3.2v3.6h-3.2z" fill="none" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/>`,
  bolt: `<path d="M13 3L5.5 13.4h4.4L9 21l7.5-10.4h-4.4z" fill="none" stroke-width="2" stroke-linejoin="round"/>`,
};
...
// ---------------------------------------------------------------------------
// Écrans recréés — viewBox 430×932
// ---------------------------------------------------------------------------
const SCR_W = 430;
const SCR_H = 932;
...
/**
 * En-tête v2 — deux styles, aucun dégradé.
 *   "surface"  : blanc, titre encre, pastille de statut émeraude,
 *                icône bleue dans un disque teinté (recommandé).
 *   "bleu-plat": aplat bleu uni, typo blanche (v1 sans le dégradé).
 */
let HEADER_STYLE = "surface";

function appHeader(title, subtitle, { trailingIcon = "shieldS" } = {}) {
  if (HEADER_STYLE === "bleu-plat") {
    return `
    <rect x="0" y="0" width="${SCR_W}" height="128" fill="${C.blue}"/>
    ${statusBar(true)}
    ${txt(28, 84, title, { size: 26, weight: 800, fill: C.white })}
    ${txt(28, 110, subtitle, { size: 14.5, weight: 500, fill: C.white, opacity: 0.85 })}
    ${icon(trailingIcon, 372, 76, 30, C.white)}`;
  }
  return `
    <rect x="0" y="0" width="${SCR_W}" height="128" fill="${C.white}"/>
    <line x1="0" y1="128" x2="${SCR_W}" y2="128" stroke="${C.border}" stroke-width="1.4"/>
    ${statusBar(false)}
    ${txt(28, 86, title, { size: 26, weight: 800, fill: C.ink })}
    <circle cx="33" cy="106" r="4" fill="${C.emerald}"/>
    ${txt(46, 111, subtitle, { size: 13.5, weight: 600, fill: C.muted })}
    <circle cx="385" cy="86" r="23" fill="${C.tint}"/>
    ${icon(trailingIcon, 371, 72, 28, C.blue)}`;
}
...
/** Écran Accueil — protection active. */
function screenHome() {
  let y = 148;
  ...
  // Carte discrète mini-jeux (comme dans l'app)
  s += card(24, y, SCR_W - 48, 64, { accent: C.amber });
  s += icon("gamepad", 44, y + 20, 26, C.amber);
  s += txt(84, y + 28, "Jeux — défis du jour", { size: 14.5, weight: 700 });
  s += txt(84, y + 48, "DEFENSE · TRACE · classement anonyme", { size: 12, fill: C.muted });
  s += icon("chevron", SCR_W - 62, y + 21, 22, C.muted);
...
// ---------------------------------------------------------------------------
// Mini-jeux — aperçus vectoriels + hub « Jeux »
// ---------------------------------------------------------------------------

/** Petit vaisseau-bouclier DEFENSE (boîte size×size). */
function gameShip(x, y, size, color = C.gameCyan) {
  const s = size / 48;
  return `<g transform="translate(${x},${y}) scale(${s})">
    <path d="M24 3l17 6.4v13c0 11.6-7.4 19-17 23.2C14.4 41.4 7 34 7 22.4v-13z" fill="${color}"/>
    <path d="M21.4 29.4l-5.4-5.4 2.7-2.7 2.7 2.7 9.6-9.6 2.7 2.7z" fill="#06233F"/>
  </g>`;
}

/** Petit fraudeur violet à cornes (TRACE). */
function villain(cx, cy, r) {
  return `<g>
    <path d="M${cx - r * 0.9} ${cy - r * 1.25} L${cx - r * 0.25} ${cy - r * 0.75} L${cx - r * 1.05} ${cy - r * 0.35} Z" fill="#8B5CF6"/>
    <path d="M${cx + r * 0.9} ${cy - r * 1.25} L${cx + r * 0.25} ${cy - r * 0.75} L${cx + r * 1.05} ${cy - r * 0.35} Z" fill="#8B5CF6"/>
    <circle cx="${cx}" cy="${cy}" r="${r}" fill="#8B5CF6"/>
    <circle cx="${cx - r * 0.38}" cy="${cy - r * 0.18}" r="${r * 0.16}" fill="${C.white}"/>
    <circle cx="${cx + r * 0.38}" cy="${cy - r * 0.18}" r="${r * 0.16}" fill="${C.white}"/>
    <path d="M${cx - r * 0.42} ${cy + r * 0.34} Q ${cx} ${cy + r * 0.72} ${cx + r * 0.42} ${cy + r * 0.34}" fill="none" stroke="${C.white}" stroke-width="${r * 0.16}" stroke-linecap="round"/>
  </g>`;
}

/** Aperçu DEFENSE : ciel étoilé, spams qui tombent, tir du bouclier. */
function defensePreview(x, y, w, h) {
  let s = `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="12" fill="${C.gameNight}"/>`;
  const stars = [
    [0.08, 0.2], [0.18, 0.62], [0.27, 0.34], [0.38, 0.14], [0.46, 0.55],
    [0.57, 0.28], [0.66, 0.7], [0.74, 0.18], [0.85, 0.44], [0.93, 0.66],
    [0.12, 0.85], [0.52, 0.82], [0.9, 0.12],
  ];
  stars.forEach(([fx, fy]) => {
    s += `<circle cx="${x + fx * w}" cy="${y + fy * h}" r="1.3" fill="#9FD0FF" opacity="0.55"/>`;
  });

  // HUD
  s += `<rect x="${x + 10}" y="${y + 8}" width="86" height="18" rx="9" fill="#0E1730"/>`;
  s += txt(x + 53, y + 20.5, "SCORE 12 480", { size: 9.5, weight: 700, fill: "#EAF0FF", anchor: "middle" });
  s += `<rect x="${x + 102}" y="${y + 8}" width="52" height="18" rx="9" fill="#0E1730"/>`;
  s += txt(x + 128, y + 20.5, "WAVE 7", { size: 9.5, weight: 700, fill: "#EAF0FF", anchor: "middle" });
  s += `<rect x="${x + w - 76}" y="${y + 8}" width="66" height="18" rx="9" fill="#0E1730"/>`;
  s += txt(x + w - 43, y + 20.5, "COMBO ×3", { size: 9.5, weight: 700, fill: C.gameAmber, anchor: "middle" });

  // Ligne de défense + vaisseau
  const ly = y + h - 20;
  s += `<line x1="${x + 12}" y1="${ly}" x2="${x + w - 12}" y2="${ly}" stroke="${C.gameCyan}" stroke-width="2" opacity="0.85"/>`;
  const sx = x + w / 2;
  s += `<circle cx="${sx}" cy="${y + h - 34}" r="20" fill="${C.gameCyan}" opacity="0.16"/>`;
  s += gameShip(sx - 14, y + h - 50, 28);

  // Tir vers un spam
  const tx = sx + 42, ty = y + 44;
  s += `<line x1="${sx}" y1="${y + h - 48}" x2="${tx}" y2="${ty + 10}" stroke="${C.gameCyan}" stroke-width="2.5" opacity="0.9" stroke-linecap="round"/>`;

  // Bulles spam (rouges) et bienveillante (verte)
  s += `<circle cx="${tx}" cy="${ty}" r="13" fill="${C.gameSpam}"/>`;
  s += icon("phone", tx - 8, ty - 8, 16, C.white);
  s += `<circle cx="${x + 58}" cy="${y + 52}" r="12" fill="${C.gameSpam}"/>`;
  s += icon("phone", x + 51, y + 45, 14, C.white);
  s += `<circle cx="${x + w - 56}" cy="${y + 60}" r="12" fill="${C.gameSafe}"/>`;
  s += icon("check", x + 49 + w - 112, y + 53, 14, "#06233F");
  return s;
}

/** Aperçu TRACE : grille colorée par régions, croix et fraudeurs. */
function tracePreview(x, y, w, h) {
  let s = `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="12" fill="${C.gameBoard}"/>`;

  s += `<rect x="${x + 10}" y="${y + 8}" width="86" height="18" rx="9" fill="#0E2438"/>`;
  s += txt(x + 53, y + 20.5, "Défi du jour", { size: 9.5, weight: 700, fill: C.gameCyan, anchor: "middle" });
  s += `<rect x="${x + 102}" y="${y + 8}" width="58" height="18" rx="9" fill="#0E2438"/>`;
  s += txt(x + 131, y + 20.5, "Grille 3/5", { size: 9.5, weight: 700, fill: "#EAF0FF", anchor: "middle" });
  s += `<rect x="${x + w - 56}" y="${y + 8}" width="46" height="18" rx="9" fill="#0E2438"/>`;
  s += txt(x + w - 33, y + 20.5, "02:14", { size: 9.5, weight: 700, fill: "#EAF0FF", anchor: "middle" });
  ...
  const cols = [C.gamePink, C.gameTeal, C.gameAmber, C.gameBlue, "#A78BFA"];
  const cell = 16, gap = 2;
  const gridW = 5 * cell + 4 * gap;
  const gx = x + (w - gridW) / 2;
  const gy = y + 30;
  const crosses = [[0, 1], [1, 3], [2, 0], [3, 4], [4, 2]];
  const villains = [[1, 1], [3, 3]];
  regions.forEach((row, r) => {
    row.forEach((reg, c) => {
      const cx = gx + c * (cell + gap);
      const cy = gy + r * (cell + gap);
      s += `<rect x="${cx}" y="${cy}" width="${cell}" height="${cell}" rx="4" fill="${cols[reg]}" fill-opacity="0.82"/>`;
    });
  });
  crosses.forEach(([r, c]) => {
    const cx = gx + c * (cell + gap) + cell / 2;
    const cy = gy + r * (cell + gap) + cell / 2;
    s += `<path d="M${cx - 3.6} ${cy - 3.6}l7.2 7.2M${cx + 3.6} ${cy - 3.6}l-7.2 7.2" stroke="${C.white}" stroke-width="2" stroke-linecap="round" opacity="0.9"/>`;
  });
  villains.forEach(([r, c]) => {
    const cx = gx + c * (cell + gap) + cell / 2;
    const cy = gy + r * (cell + gap) + cell / 2;
    s += villain(cx, cy + 1, 6);
  });
...
/** Chip streak ambre (série de jours). */
function streakChip(x, y, label) {
  const w = 30 + label.length * 8;
  return `<rect x="${x - w}" y="${y}" width="${w}" height="24" rx="12" fill="#FDF1DC"/>
    ${icon("bolt", x - w + 6, y + 3, 18, "#B45309")}
    ${txt(x - 10, y + 17, label, { size: 12.5, weight: 800, fill: "#B45309", anchor: "end" })}`;
}

/** Écran hub « Jeux » (GamesScreen). */
function screenGames() {
  let s = `<rect width="${SCR_W}" height="${SCR_H}" fill="#F7F9FE"/>`;
  s += appHeader("Jeux", "Un défi par jour, ton classement", { trailingIcon: "trophy" });
  let y = 148;

  // Carte DEFENSE
  s += card(24, y, SCR_W - 48, 264, { accent: C.coral });
  s += defensePreview(44, y + 16, SCR_W - 88, 120);
  s += txt(44, y + 164, "DEFENSE", { size: 17, weight: 800 });
  s += streakChip(SCR_W - 44, y + 148, "6 j");
  s += txt(44, y + 186, "Arcade — détruis les appels spam.", { size: 12.5, fill: C.muted });
  s += txt(SCR_W - 44, y + 186, "Record : 12 480 pts", { size: 12, weight: 600, fill: C.muted, anchor: "end" });
  s += `<rect x="44" y="${y + 202}" width="${SCR_W - 88}" height="44" rx="22" fill="${C.coral}"/>`;
  s += txt(SCR_W / 2, y + 230, "Jouer le défi du jour ›", { size: 14.5, weight: 700, fill: C.white, anchor: "middle" });
  y += 280;

  // Carte TRACE
  s += card(24, y, SCR_W - 48, 264, { accent: C.blue });
  s += tracePreview(44, y + 16, SCR_W - 88, 120);
  s += txt(44, y + 164, "TRACE", { size: 17, weight: 800 });
  s += streakChip(SCR_W - 44, y + 148, "12 j");
  s += txt(44, y + 186, "Puzzle — démasque le fraudeur caché.", { size: 12.5, fill: C.muted });
  s += txt(SCR_W - 44, y + 186, "Record : 9 350 pts", { size: 12, weight: 600, fill: C.muted, anchor: "end" });
  s += `<rect x="44" y="${y + 202}" width="${SCR_W - 88}" height="44" rx="22" fill="${C.blue}"/>`;
  s += txt(SCR_W / 2, y + 230, "Jouer le défi du jour ›", { size: 14.5, weight: 700, fill: C.white, anchor: "middle" });
  y += 292;

  s += txt(SCR_W / 2, y, "Nouveau défi chaque jour · 3 essais classés · 100 % anonyme", { size: 11.5, fill: C.muted, anchor: "middle" });

  s += bottomNav(3);
  return s;
}

/** Écran d'appel entrant bloqué. */
function screenCallBlocked({ legit = false } = {}) {
  let s = `<rect width="${SCR_W}" height="${SCR_H}" fill="${C.nightDark}"/>`;
  s += statusBar(true);
...
/** Écran vérification d'un numéro. */
function screenLookup() {
  ...
  s += `<path d="M ${gx - 78} ${gy + 30} A 84 84 0 1 1 ${gx + 66} ${gy + 46}" fill="none" stroke="${C.coral}" stroke-width="15" stroke-linecap="round"/>`;
...
function phone(screenSvg, x, y, w, { idSuffix = "" } = {}) {
  const bezel = w * 0.032;
  const sw = w - bezel * 2;
...
// ---------------------------------------------------------------------------
// Compositions store — fonds en aplat, halos plats conservés
// ---------------------------------------------------------------------------
...
/** Direction A — fond bleu ciel uni, titre haut, téléphone plein pied. */
function slideA({ W, H, title, sub, screen, badge = null }) {
  const lines = Array.isArray(title) ? title : [title];
  const tSize = 74;
  let s = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">`;
  s += `<rect width="${W}" height="${H}" fill="${C.skyFlat}"/>`;
...
/** Slide marque — fond nuit uni. */
function slideABrand({ W, H }) {
  const extra = H - 1920;
  let s = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">`;
  s += `<rect width="${W}" height="${H}" fill="${C.night}"/>`;
  ...
  s += shieldLogo(W / 2 - 130, y - 10, 260, { bg: C.night, fg: C.white });
  y += 340;
  ...
  const items = [
    ...
    ["gamepad", "2 mini-jeux inclus", "DEFENSE & TRACE · un défi chaque jour."],
  ];
  const cw = W - 220;
...
/** Direction B — éditorial bleu nuit uni. */
function slideB({ W, H, kicker, lines, bullets = [], screen = null, footer = "who-called.com" }) {
  ...
  if (screen) {
    ...
    if (bullets.length) {
      ...
      s += `<rect x="0" y="${H - bh}" width="${W}" height="${bh}" fill="${C.nightBand}"/>`;
      let by = H - bh + 96;
    ...
    s += shieldLogo(W / 2 - 150, y + 90, 300, { bg: C.night, fg: C.white });
...
// ---------------------------------------------------------------------------
// Feature graphics Play Store 1024×500 — aplats
// ---------------------------------------------------------------------------

function featureGraphicA() {
  const W = 1024, H = 500;
  let s = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">`;
  s += `<rect width="${W}" height="${H}" fill="${C.blue}"/>`;
  ...
  s += txt(64, 380, "Liste officielle ARCEP + communauté.", { size: 25, fill: C.white, opacity: 0.85 });
  s += txt(64, 416, "Et 2 mini-jeux — un défi chaque jour.", { size: 25, fill: "#FDE68A", opacity: 0.95 });
...
function featureGraphicB() {
  ...
  s += shieldLogo(730, 96, 220, { bg: C.night, fg: C.white });
  ...
  const bullets = [
    ...
    "Anonyme — aucun compte",
    "Liste ARCEP + communauté",
    "2 mini-jeux · un défi par jour",
  ];
  let y = 348;
  bullets.forEach((b) => {
...
const SIZES = {
  android: { W: 1080, H: 1920, outW: 1080, outH: 1920 },
  ios: { W: 1080, H: 2347, outW: 1320, outH: 2868 },
};
...
function buildSlides({ W, H }) {
  return [
    ...
    ["07-jeux", slideA({
      W, H,
      title: ["Deux mini-jeux", "pour souffler"],
      sub: "DEFENSE & TRACE · un défi chaque jour",
      screen: screenGames(),
    })],
    ...
    ["B1-hero", slideB({
      ...
      bullets: [
        ...
        ["2 mini-jeux · un défi par jour", false],
      ],
    })],
    ["B2-eviter", slideB({
      W, H,
    ...
    ["B4-jeux", slideB({
      W, H,
      kicker: "BONUS",
      lines: ["Et deux", "mini-jeux."],
      bullets: [
        ["DEFENSE — arcade anti-spam", true],
        ["TRACE — puzzle de déduction", false],
      ],
      screen: screenGames(),
    })],
  ];
...
// Comparatif des deux styles de header (android, slide 02) ---------------
for (const style of ["surface", "bleu-plat"]) {
  HEADER_STYLE = style;
  const svg = slideA({
    W: 1080, H: 1920,
    title: ["Protégé en", "permanence"],
  ...
  const svgPath = join(SVG_DIR, `header-alt-${style}.svg`);
  writeFileSync(svgPath, svg);
  execSync(`rsvg-convert -w 1080 -h 1920 "${svgPath}" -o "${join(PNG_DIR, "android", `header-alt-${style}.png`)}"`);
  rendered++;
}
HEADER_STYLE = "surface";

console.log(`OK — ${rendered} PNG rendus dans store/v3/png/ (SVG maîtres dans store/v3/svg/)`);
